...
static int cpuinfo_arm(FILE *file, odp_system_info_t *sysinfo)
{
	...
	if (part == ARM_CORTEX_A53)
		...
		strcat(sysinfo->model_str, ARM_CORTEX_A57_INFO);
	else if (part == ARM_CORTEX_A72)
		strcat(sysinfo->model_str, ARM_CORTEX_A72_INFO);
	else
		strcat(sysinfo->model_str, "UNKNOWN");
...
int odp_cpu_count(void)
{
	...
	/* For GPP proof point performance benchmarking, we are isolating
	   the CPUs so, hard coding is required to use isolated cores */
		return ODPFSL_MAX_PLATFORM_CORE;
#else
