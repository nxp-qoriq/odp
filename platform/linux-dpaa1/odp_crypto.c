...
static enum qman_cb_dqrr_result crypto_ipsec_dqrr_cb_inp(struct qman_fq *fq,
		const struct qm_dqrr_entry *dqrr,
		uint64_t *user_context)
{
	assert(dqrr->stat & QM_DQRR_STAT_FD_VALID);
...
static enum qman_cb_dqrr_result sk_dqrr_cb(struct qman_fq *fq ODP_UNUSED,
				const struct qm_dqrr_entry *dqrr ODP_UNUSED,
				uint64_t *user_context ODP_UNUSED)
{
	return qman_cb_dqrr_consume;
}
					...
					odp_ipsec_params_t *ipsec_params)
{
	...
	out_qentry = queue_to_qentry(ses->s.compl_queue);
	/* Set completion callback */
	out_qentry->s.fq.cb.dqrr_ctx = crypto_ipsec_dqrr_cb_inp;
