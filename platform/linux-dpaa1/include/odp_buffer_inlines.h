...
static inline odp_buffer_t odp_buffer_encode_handle(odp_buffer_hdr_t *hdr)
{
	...
	if (bman[pool_id])
		handle.index = ((intptr_t)hdr) / ODP_CACHE_LINE_SIZE;
	else
					    ...
					    odp_buffer_seg_t seg)
{
	...
	/* if segcount includes also the sg table buffer, decrement it by 2 */
	count = (buf->segcount > 1) ? buf->segcount - 2 : buf->segcount - 1;

	if (seg == ODP_SEGMENT_INVALID ||
	    seghandle.prefix != buf->handle.prefix || seghandle.seg >= count)
