...
bool bman[ODP_CONFIG_POOLS] = {false};
...
int odp_pool_init_global(void)
{
	...
	shm = odp_shm_reserve(SHM_DEFAULT_NAME,
			      sizeof(pool_table_t),
			      sizeof(pool_entry_t), ODP_SHM_SW_ONLY);
...
odp_pool_t odp_pool_create(const char *name, odp_pool_param_t *params)
{
	...
	if (params->buf.num > ODP_BUFFER_MAX_BUFFERS)
		return ODP_POOL_INVALID;
	...
	for (i = 0; i < ODP_CONFIG_POOLS; i++) {
		...
		if (bman[pool->s.pool_id]) {
			/* BMAN based pool */
			pool->s.pool_mdata_addr = 0;
			pool_hdl = odp_pool_create_bman(pool);
			return pool_hdl;
		}
...
void odp_buffer_free(odp_buffer_t buf)
{
	...
	if (inq != ODP_QUEUE_INVALID) {
		...
		if (qentry->s.type != ODP_QUEUE_TYPE_PLAIN &&
		    odp_queue_sched_type(inq) == ODP_SCHED_SYNC_ORDERED) {
			struct qm_fd fd;
			do {
				ret = qman_enqueue_orp(&qentry->s.orp_fq,
						    &fd, QMAN_ENQUEUE_FLAG_HOLE,
						       &qentry->s.orp_fq,
						       buf_hdr->orp.seqnum);
			} while (ret == -EBUSY);
		}
	...
	if (buf_hdr->type == ODP_EVENT_PACKET) {
		odp_buffer_free_bman(buf_hdr, pool);
		return;
	}
