...
# burst mode
./odp_tm -ipcap:in=${PCAP_IN}:out=${PCAP_OUT} -t 5 -m 0
STATUS=$?
...
# queue mode
./odp_tm -ipcap:in=${PCAP_IN}:out=${PCAP_OUT} -t 5 -m 1
STATUS=$?
PCAP_OUT_SIZE=`stat -c %s ${PCAP_OUT}`
rm -f ${PCAP_OUT}
...
# sched/queue mode
./odp_tm -ipcap:in=${PCAP_IN}:out=${PCAP_OUT} -t 5 -m 2
STATUS=$?
PCAP_OUT_SIZE=`stat -c %s ${PCAP_OUT}`
rm -f ${PCAP_OUT}
...
./odp_tm -ipcap:in=${PCAP_IN}:out=${PCAP_OUT} -t 5 -m 0 -c 1
STATUS=$?
PCAP_OUT_SIZE=`stat -c %s ${PCAP_OUT}`
rm -f ${PCAP_OUT}
