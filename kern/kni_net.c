...
kni_net_rx_normal(struct kni_dev *kni)
{
	...
	num = kni_fifo_get(kni->rx_q, (void **)va, num_rx);
	if (num == 0)
		return;

	KNI_DBG("\nRX pacekt num = %d  num_rx = %d", num, num_rx);
	/* Transfer received packets to netif */
	for (i = 0; i < num; i++) {
		if (va[i] == NULL)
			continue;
		kva = (void *)va[i] - kni->kbuf_va + kni->kbuf_kva;
		KNI_DBG("kva is %p, len = %d", kva, kva->data_len);
		len = kva->data_len;
		data_kva = kva->data - kni->kbuf_va + kni->kbuf_kva;
#ifdef NCS_DEBUG
		hexdump(data_kva, 64);
#endif
	...
	/* Burst enqueue kbufs into free_q */
...
kni_net_rx_lo_fifo(struct kni_dev *kni)
{
	...
	if (ret) {
		...
		for (i = 0; i < num; i++) {
			kva = (void *)va[i] - kni->kbuf_va + kni->kbuf_kva;
			len = kva->pkt_len;
			data_kva = kva->data - kni->kbuf_va +
						kni->kbuf_kva;

			alloc_kva = (void *)alloc_va[i] - kni->kbuf_va +
							kni->kbuf_kva;
			alloc_data_kva = alloc_kva->data - kni->kbuf_va +
								kni->kbuf_kva;
		...
		/* Burst enqueue kbufs into tx_q */
		ret = kni_fifo_put(kni->tx_q, (void **)alloc_va, num);
		if (ret != num)
			/* Failing should not happen */
			KNI_ERR("Fail to enqueue kbufs into tx_q\n");
	}
	...
	if (ret != num)
		/* Failing should not happen */
		KNI_ERR("Fail to enqueue kbufs into free_q\n");
...
kni_net_rx_lo_fifo_skb(struct kni_dev *kni)
{
	...
	/* Burst dequeue kbufs from rx_q */
	ret = kni_fifo_get(kni->rx_q, (void **)va, num);
	if (ret == 0)
		return;

	/* Copy kbufs to sk buffer and then call tx interface */
	for (i = 0; i < num; i++) {
		kva = (void *)va[i] - kni->kbuf_va + kni->kbuf_kva;
		len = kva->data_len;
		data_kva = kva->data - kni->kbuf_va + kni->kbuf_kva;
	...
	/* enqueue all the kbufs from rx_q into free_q */
	ret = kni_fifo_put(kni->free_q, (void **)&va, num);
	if (ret != num)
		/* Failing should not happen */
		KNI_ERR("Fail to enqueue kbufs into free_q\n");
...
kni_net_tx(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (likely(ret == 1)) {
		...
		pkt_kva->ol_info = skb_shinfo(skb)->gso_size; /* passes gso_size from Kernel to GPP */

		/* enqueue kbuf into tx_q */
		ret = kni_fifo_put(kni->tx_q, (void **)&pkt_va, 1);
		if (unlikely(ret != 1)) {
			/* Failing should not happen */
			KNI_ERR("Fail to enqueue kbuf into tx_q\n");
			goto drop;
		}
	} else {
		/* Failing should not happen */
		KNI_ERR("Fail to dequeue kbuf from alloc_q\n");
		goto drop;
	}
